...
export const Textarea = ({id, placeholder, className, name, control, rules, error, onChange, readOnly, children}) => {
	...
	if(control != undefined) {
		input = <Controller
					...
					render={({ field }) => (
						<textarea 
							className={`textarea textarea-bordered w-full text-gray-900 text-sm !focus:ring-0 !focus:ring-offset-0 focus:border-2 ${className ?? ''} ${error ? 'border-red-400 focus:ring-red-600' : ''}`}
							placeholder={placeholder} 
							col="40"
							rows="5"
							{...field}
		...
		input = <textarea 
					className={`textarea textarea-bordered text-gray-900 text-sm !focus:ring-0 !focus:ring-offset-0 focus:border-2 ${className ?? ''}`} 
					placeholder={placeholder} 
					col="20"
					rows="5"
					onChange={onChange}
