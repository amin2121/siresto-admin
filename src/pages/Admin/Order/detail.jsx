...
import { rupiahToNumber, rupiah, baseUrl, capitalize } from '../../../utils/strings'
...
export default function Detail() {
    ...
    const mutationUbahStatusOrder = useMutation(async (data) => {
        ...
        const response = await axios.patch('order/ubah-status-order/' + state.id, data, config)
        const res = response.data
    ...
    const ubahStatusOrder = (status) => {
        if(statusBayar == 'not_paid') {
            if(status == 'closed') {
                ...
                const confirm = swConfirm('Apakah Anda Yakin', 'Ingin Mengubah Status Order', 'Iya, Saya Ubah')
                confirm.then(result => {
                            ...
                            <div className="w-full pl-3 pr-1 mt-3 order-list__container">
                                <div className='flex justify-between items-center order-list__title border-b border-zinc-200 pb-4 mx-4 flex-wrap'>
                                    <div class="space-x-2 flex justify-start items-center">
                                        <Link to="/order"><BiArrowBack size="20" className="text-blue-500"/></Link>
                                        <h2 className='text-black text-xl font-semibold'>Order Detail / {state.no_transaksi}</h2>
                                    </div>
                                    <p className='text-lg text-black'>{state != null ? produk.length : 0} Item</p>
                                </div>
                                ...
                                <div className="order-list__content grid grid-cols-7 mt-5 px-4">
                                    <table className='table-fixed w-full col-span-full'>
                                        ...
                                        <tbody>
                                            {produk != null && produk.map((obj, key) => (
                                                <tr key={obj.id} className="border-b border-slate-200">
                                                    <td>
                                                        <div className='flex items-center space-x-3'>
                                                            <img src={baseUrl + obj.produk.gambar} alt={obj.produk.gambar} className="h-16 w-16 object-cover rounded" />
                                                            <div className='justify-items-center'>
                                                                <h5 className='text-md text-black'>{obj.produk.nama_produk}</h5>
                                                                <span className='text-xs text-blue-400'>Makanan Pedas</span>
                                                                <span className="text-xs text-black-400 ml-3">x{obj.jumlah_beli}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="text-right">
                                                        <span className='inline-block mr-1 text-sm font-bold text-blue-500'>IDR {rupiah(obj.total_harga_jual)}</span>
                                                        {obj.produk.diskon > 0 && <span className="text-xs text-slate-300 block">- IDR {rupiah(obj.produk.diskon * obj.jumlah_beli)}</span>}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    ...
                                    <div className="flex justify-end col-span-full border-b border-slate-200">
                                        <div className="w-1/2 bg-white sticky bottom-0 left-0 pl-6 py-4 space-y-3 mt-5">
                                            ...
                                            <div className="flex justify-between">
                                                ...
                                                <p className="font-bold text-blue-500">Rp. {rupiah(subtotal)}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center col-span-full pt-5 pb-5 border-b border-slate-200 flex-wrap space-y-2"> 
                                        ...
                                        <div className="space-x-2 order__status-container text-sm flex overflow-x-auto overflow-y-hidden"> 
                                            <span className={`order__status-item ${statusOrder == 'open' && 'active'}`} onClick={() => ubahStatusOrder('open')}>Open</span>
                                            <span className={`order__status-item ${statusOrder == 'in_progress' && 'active'}`} onClick={() => ubahStatusOrder('in_progress')}>In Progress</span>
                                            <span className={`order__status-item ${statusOrder == 'closed' && 'active'}`} onClick={() => ubahStatusOrder('closed')}>Closed</span>
                                        </div> 
                                    </div> 
                        ...
                        <div className="rounded md:col-span-5 col-span-12 md:right-0 flex flex-col px-2">
                            <div className="border border-slate-200 rounded p-4">
                                ...
                                <div className="flex justify-between text-xs text-slate-500">
                                    <h1>Status Bayar</h1>
                                    <p className="font-semibold text-sm">{statusBayar == 'already_paid' ? <Badge title="Sudah Bayar" type="success" /> : <Badge title="Belum Bayar" type="error" />}</p>
                                </div>
                            </div>
                            <div className={`border border-slate-200 rounded p-4 pb-6 mt-4 ${statusBayar == 'already_paid' && 'hidden'}`}>
                                <h1 className="text-sm text-black font-semibold mb-2">Pembayaran</h1>
                                <div className="order-pembayaran__form">
                                    <div className="w-full">
                                        <label className="label">
                                            <span className="label-text text-xs">Metode Pembayaran</span>
                                        </label>
                                        <MetodePembayaran />
                                    </div>
                                    ...
                                    <div class="w-full">
                                        ...
                                        <InputGroupCurrency type="text" id="input-nilai-pembayaran" directionIcon="left" name="pembayaran" icon='IDR' onChange={hitungKembalian} placeholder="Pembayaran" control={control} error={errors.pembayaran ? true : false}/>
                                    </div>
                            ...
                            <div className={`border border-slate-200 rounded p-4 mt-4 ${statusBayar == 'not_paid' && 'hidden'}`}>
                                <h1 className="text-sm text-black font-semibold mb-2">Detail Pembayaran</h1>
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Nilai Transaksi</h1>
                                    <p className="font-semibold text-sm">{rupiah(state.nilai_transaksi)}</p>
                                </div>
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Metode Pembayaran</h1>
                                    <p className="font-semibold text-sm">{capitalize(state.metode_pembayaran || '')}</p>
                                </div>
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Pajak</h1>
                                    <p className="font-semibold text-sm">{rupiah(state.pajak || 0)}</p>
                                </div>
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Service Charge</h1>
                                    <p className="font-semibold text-sm">{rupiah(state.service_charge || 0)}</p>
                                </div>
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Pembayaran</h1>
                                ...
                                <div className="flex justify-between text-xs text-slate-500 mb-2">
                                    <h1>Kembali</h1>
                                    <p className="font-semibold text-sm">{rupiah(state.kembali)}</p>
                                </div>
                            </div>
                        </div>
                    </div>
