...
export default function Pembayaran() {
    ...
    const { register, handleSubmit, formState: { errors }, reset, clearErrors, control, setValue, getValues } = useForm();
    ...
    const tambahJumlahBeli = (id) => {
        ...
        if(cart !== undefined) {
            ...
            produkBaru[index]['jumlah_produk'] = parseInt(produkBaru[index]['jumlah_produk']) + 1

            let jumlahProduk = produkBaru[index]['jumlah_produk'] == '' ? 0 : produkBaru[index]['jumlah_produk']
    ...
    const hitungTotal = () => {
        setDiskon(produk.length > 0 ? produk.reduce((n, {harga_total_diskon}) => n + harga_total_diskon, 0) : 0)
        setSubtotal(produk.length > 0 ? produk.reduce((n, {harga_total}) => n + harga_total, 0) : 0)
    }
                                    ...
                                    <table className='table-auto w-full'>
                                        <thead>
                                            <tr>
                                                ...
                                                <th className='text-slate-400 uppercase text-xs text-left pb-5'>Jumlah Beli</th>
                                                <th className='text-slate-400 uppercase text-xs text-left pb-5'>Harga</th>
                                                <th className='text-slate-400 uppercase text-xs text-left pb-5'>Total</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                                ...
                                                <tr key={obj.id}>
                                                    <td>
                                                        <div className='flex items-center space-x-3'>
                                                            <img src={baseUrl + obj.gambar} alt={obj.gambar} className="h-20 w-20 object-cover rounded" />
                                                            <div className='justify-items-center'>
                                                                <h5 className='text-md text-black'>{obj.nama_produk}</h5>
                        ...
                        <div className="lg:w-full lg:col-span-4 col-span-12 bg-slate-100 rounded flex flex-col px-6">
                            ...
                            <div className="order-pembayaran__form">
                                <div className="w-full">
                                    ...
                                    <Input type="text" placeholder="Nama Pelanggan" name="nama_pelanggan" control={control} error={errors.nama_pelanggan ? true : false}/>
                                </div>
