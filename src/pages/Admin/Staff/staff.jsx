...
const Staff = () => {
  ...
  const fetchData = async () => {
    ...
    const response = await axios.get(
      `staff?s=${keyword}&limit=${limit}&sort=DESC&page=${page}`
    );
            ...
            {isFetching ? (
              ...
              data?.map((obj, key) => (
                <tr className="bg-white border-b border-blue-200" key={key}>
                  <td className="py-4 px-6 text-sm">{++key}</td>
                  <td className="py-4 px-6 whitespace-nowrap text-sm">
                    {obj.name}
                  </td>
                  <td className="py-4 px-6 text-sm">{obj.username}</td>
                  <td className="py-4 px-6 text-sm">{obj.no_telepon}</td>
