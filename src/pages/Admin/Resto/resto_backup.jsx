...
const Resto = () => {
  ...
  const fetchData = async () => {
    const response = await axios.get(
      `resto?s=${keyword}&limit=${limit}&sort=DESC&page=${page}`
    );
  ...
  const handleOutside = (e) => {
    if (btnDropdownAksi?.current?.contains(e.target) != null) {
      if (!btnDropdownAksi?.current?.contains(e.target)) {
        setShowDropdownAksi({
          ...showDropdownAksi,
          status: !showDropdownAksi.status,
        });
      }
    }
  };
            ...
            {isFetching ? (
              ...
              data?.map((obj, key) => (
                <tr className="bg-white border-b border-blue-200" key={key}>
                  ...
                  <td className="py-4 px-6 text-sm">{obj.nama_pemilik}</td>
                    ...
                    <div className="grid grid-cols-2">
                      <div className="dropdown dropdown-end">
                        <Button
                          ...
                          color="ghost"
                          size="sm"
                          startIcon={<BsThreeDots size={16} />}
                          loading={false}
                          onClick={() => menuAksiRow(obj.id)}
                          ref={btnDropdownAksi}
                        />
                        <ul
                          tabIndex={0}
                          id={`dropdown-aksi-${obj.id}`}
                          ref={elementDropdownAksi}
                          className={`dropdown-content !fixed menu p-2 shadow bg-base-100 rounded-box w-52 ${
                            showDropdownAksi.status == false ? "hidden" : ""
                          }`}
                          style={{ zIndex: "100000000000 !important" }}
                        >
                          <li>
                            <a
                              onClick={() => {
                                setIsShowModal();
                                setValue("id", obj.id);
                                setValue("is_enabled", obj.status_resto);
                              }}
                            >
                              Ganti Status
                            </a>
                          ...
                          <li>
                            <Link to="/admin/resto/edit" state={obj}>
                              Edit
                            </Link>
                          </li>
                          <li>
                            <a onClick={() => confirmDeleteResto(obj.id)}>
                              Hapus
                            </a>
                          </li>
                        </ul>
              ...
              <td colSpan="6" className="text-right">
                <div className="flex space-x-1 justify-end">
                  <div>
                    Baris Per Halaman :
                    <select
                      className="select select-ghost w-24 focus:bg-opacity-0 focus:outline-0"
