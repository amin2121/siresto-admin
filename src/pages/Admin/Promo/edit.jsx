...
import { InputGroup, Input, InputCurrency, MessageError, Textarea } from '../../../components/Input'
...
const Edit = () => {
	...
	useEffect(() => {
		...
		setValue('tanggal_awal_promo', moment(data.tanggal_awal_promo).format('YYYY-MM-DD'))
    ...
    return (
        <>
        	<HeaderContent linkBack="/promo" title="Edit Promo" breadcrumbs={breadcrumbs}></HeaderContent>
			    		...
			    		<input type="hidden" name="id" {...register("id")} defaultValue={data.id} />
				      	...
				      	<div className="grid grid-cols-8 gap-1 md:gap-4 mb-4">
				      		...
				      		<div className="relative col-span-12 md:col-span-4">
				      			<Input type="text" placeholder="Judul Promo" name="judul_promo" control={control} rules={{required: true}} error={errors.judul_promo ? true : false} defaultValue={data.judul_promo}/>
				      			{errors?.judul_promo && <MessageError>Judul Promo Tidak Boleh Kosong</MessageError>}
				      		</div>
								</div>
					        ...
					        <div className="relative col-span-12 md:col-span-4">
				      		...
				      		<div className="relative col-span-12 md:col-span-4">
				      			<Input type="date" placeholder="Tanggal Awal Promo" name="tanggal_awal_promo" control={control} rules={{required: true}} error={errors.tanggal_awal_promo ? true : false}/>
				      			{errors?.tanggal_awal_promo && <MessageError>Tanggal Awal Promo Tidak Boleh Kosong</MessageError>}
				      		</div>
								</div>
				      		...
				      		<div className="relative col-span-12 md:col-span-4">
					  			<InputGroup icon="Hari" directionIcon="right" type="number" placeholder="Periode Promo" name="periode_promo" rules={{required: true}} control={control} error={errors.periode_promo ? true : false}/>
				      			{errors.periode_promo?.type === 'required' && <MessageError>Periode Promo Tidak Boleh Kosong</MessageError>}
				      		</div>
								</div>
	      					...
	      					<div className="relative col-span-12 md:col-span-4 flex">
						  			...
						  			<div className="form-control flex-1 mr-4">
										  <label className="label cursor-pointer">
										    ...
										    <input type="radio" name="status_promo" {...register('status_promo')} value="nominal" className="radio radio-secondary"/>
										  </label>
										</div>
										...
										<div className="form-control flex-1">
										  <label className="label cursor-pointer">
										    ...
										    <input type="radio" name="status_promo" {...register('status_promo')} value="persentase" className="radio radio-secondary"/>
										  </label>
										</div>
			      			</div>
								</div>
				      		...
				      		<div className="relative col-span-12 md:col-span-4">
					  			<InputCurrency type="text" placeholder="Promo" name="nominal_promo" control={control} required="true" error={errors.nominal_promo ? true : false}/>
				      			{errors?.nominal_promo && <MessageError>Nominal Promo Tidak Boleh Kosong</MessageError>}
				      		</div>
								</div>
						    ...
						    <div className="grid grid-cols-8 gap-1 md:gap-4 mb-4">
				      		...
				      		<div className="relative col-span-12 md:col-span-4">
					  				<Textarea placeholder="Deskripsi Promo" name="deskripsi_promo" control={control} error={errors.deskripsi_promo ? true : false} className="w-full"></Textarea>
				      			{errors?.deskripsi_promo && <MessageError>Deskripsi Promo Tidak Boleh Kosong</MessageError>}
				      		</div>
								</div>
				      		...
				      		<div className="relative col-span-12 gap-x-2 md:col-span-4 md:col-start-3">
						        ...
						        <Button className="text-xs" color="ghost" startIcon={<FiXCircle size={20}/>} type="button" onClick={() => navigate('/promo')} title="Kembali" />
				      		</div>
								</div>
