...
const Produk = () => {
  ...
  const fetchData = async () => {
    const response = await axios.get(
      `meja?s=${keyword}&page=${page}&limit=${2}`
    );
        ...
        <div className="grid grid-cols-8 gap-4 mb-6">
          <div className="relative col-span-3">
            ...
            <input
              type="text"
              id="input-search"
              className={`bg-white border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 col-span-2`}
            ...
            <tbody>
              {isFetching ? (
                <tr className="bg-white border-b">
                  <td colSpan="6" className="text-center py-6 px-6">
                    <LoadingTable />
                  </td>
                </tr>
                ...
                data.result?.map((obj, key) => (
                  <tr className="bg-white border-b" key={key}>
                    ...
                    <td className="py-4 px-6">{obj.nama_produk}</td>
                    <td className="py-4 px-6">{obj.nomor_sku}</td>
                    <td className="py-4 px-6">{rupiah(obj.harga_produk)}</td>
                    <td className="py-4 px-6">
                      {obj.is_enabled == "1" ? (
                        <Badge color="success" className="text-xs text-white">
                          Aktif
                        </Badge>
                      ) : (
                        <Badge color="error" className="text-xs text-white">
                          Tidak Aktif
                        </Badge>
                      )}
                    </td>
