...
export const LaporanStok = React.forwardRef((props, ref) => {
	moment.locale('id')
  ...
  const [selectedItem, setSelectedItem] = useState(null);
	...
	return (
		<div className="text-black mt-5 px-4 mb-5" ref={ref}>
			<h1 className="text-xl text-center font-bold mt-5">SiResto</h1>
			<h2 className="text-md text-center font-semibold mb-4">Laporan Stok</h2>
				...
				<table className="w-full text-sm border-collapse border border-slate-400 table-auto mb-5">
					<thead>
						<tr className="text-sm font-semibold tracking-wide border-b border-slate-400">
							...
							<th className="px-4 py-3 border border-slate-400">Harga Beli</th>
							<th className="px-4 py-3 border border-slate-400">Harga Jual</th>
							<th className="px-4 py-3 border border-slate-400">Stok saat ini</th>
							<th className="px-4 py-3 border border-slate-400">Nilai</th>
						</tr>
					</thead>
					<tbody>
						{props?.data.map((item, key) => {
							return (
								<tr key={key}>
									<td className="text-center px-4 py-3 border border-slate-400">{item.nomor_sku ?? '-'}</td>
									...
									<td className="text-center px-4 py-3 border border-slate-400">Rp. {rupiah(item.harga_awal)}</td>
									<td className="text-center px-4 py-3 border border-slate-400">Rp. {rupiah(item.harga_jual)}</td>
									<td className="text-center px-4 py-3 border border-slate-400">{item.stok}</td>
									<td className="text-center px-4 py-3 border border-slate-400">Rp. {rupiah(item.nilai_transaksi)}</td>
								</tr>
							)
						})}
					</tbody>
