...
const produkKeranjang =
  JSON.parse(localStorage.getItem("produkKeranjang")) || [];
  ...
  reducers: {
    ...
    tambahProduk: (state, action) => {
      ...
      state.produkKeranjang.push({
        ...
        jumlah: 1,
        diskon: item.diskon,
        laba: item.harga_jual - item.harga_awal,
      });
      localStorage.setItem(
        "produkKeranjang",
        JSON.stringify(state.produkKeranjang)
      );
    },
    tambahJumlahBeliProduk: (state, action) => {
      let indexProduk = state.produkKeranjang.findIndex(
    ...
    hapusProduk: (state, action) => {
      let dataBaru = state.produkKeranjang.filter(
